...
- The reported pink/red shape above `INV` was traced to Diablo's original equipped-item durability warning: it intentionally appears when equipped gear has five durability or less and remains unchanged. Separately, gamepad modifier hints now render only while the active input device is an actual gamepad; touch, keyboard, and pointer frames clear stale gamepad-hint state.
- Gold withdrawal, gold splitting, hero naming, and other edit fields already bracket their prompts with SDL text-input start/stop calls. SDL's UIKit backend requests first-responder input at prompt start, allowing iPadOS to show the software keyboard only when an attached hardware keyboard is not serving the field.
...
- Open stash withdrawal and gold-splitting prompts first with the Magic Keyboard attached, then detached, then reattached. Hardware typing must work without a redundant software keyboard, while touch-only entry must present and dismiss the native iPad keyboard.
- Confirm the original low-durability warning appears for equipped gear at five durability or less, disappears after repair, and reports the matching item state in inventory details. Also confirm gamepad modifier hints remain limited to a paired game controller.
