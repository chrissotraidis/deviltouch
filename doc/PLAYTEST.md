...
- A gamepad hot-spell modifier could remain latched and draw the selected Repair skill as a floating pink/red icon even while the optional touch overlay was disabled. On iPad, gamepad modifier hints now render only while the active input device is an actual gamepad; touch, keyboard, and pointer frames also clear stale hint state.
...
- Confirm the pink/red Repair hot-spell hint cannot appear during touch, keyboard, mouse, or trackpad play, while its original hold behavior remains available with a paired game controller.
